...
#[derive(Debug)]
struct BaudRate(r4dcb08_lib::BaudRate);

impl BaudRate {
    pub fn iter() -> core::array::IntoIter<Self, 5> {
        [
            Self(r4dcb08_lib::BaudRate::B1200),
            Self(r4dcb08_lib::BaudRate::B2400),
            Self(r4dcb08_lib::BaudRate::B4800),
            Self(r4dcb08_lib::BaudRate::B9600),
            Self(r4dcb08_lib::BaudRate::B19200),
        ]
        .into_iter()
    }

    pub fn from(baud_rate: r4dcb08_lib::BaudRate) -> Self {
        Self(baud_rate)
    }

    pub fn from_u32(baud_rate: u32) -> Result<Self> {
        match baud_rate {
            1200 => Ok(Self(r4dcb08_lib::BaudRate::B1200)),
            2400 => Ok(Self(r4dcb08_lib::BaudRate::B2400)),
            4800 => Ok(Self(r4dcb08_lib::BaudRate::B4800)),
            9600 => Ok(Self(r4dcb08_lib::BaudRate::B9600)),
            19200 => Ok(Self(r4dcb08_lib::BaudRate::B19200)),
            _ => bail!(
    ...
    pub fn as_u32(&self) -> u32 {
        match self.0 {
            r4dcb08_lib::BaudRate::B1200 => 1200,
            r4dcb08_lib::BaudRate::B2400 => 2400,
            r4dcb08_lib::BaudRate::B4800 => 4800,
            r4dcb08_lib::BaudRate::B9600 => 9600,
...
impl Deref for BaudRate {
    type Target = r4dcb08_lib::BaudRate;
...
pub enum CliConnection {
    ...
    RTU {
        ...
        #[arg(long, default_value_t = BaudRate::from(*r4dcb08_lib::FACTORY_DEFAULT_BAUD_RATE).as_u32())]
        baud_rate: u32,

        /// RS485 address from 1 to 247
        #[arg(short, long, default_value_t = r4dcb08_lib::FACTORY_DEFAULT_ADDRESS, value_parser=maybe_hex::<u8>)]
        address: u8,
...
fn rtu_scan(device: &String, baud_rate: &BaudRate, args: &CliArgs) -> Result<u8> {
    let mut d = R4DCB08::new(
        tokio_modbus::client::sync::rtu::connect_slave(
            ...
            tokio_modbus::Slave(r4dcb08_lib::READ_ADDRESS_BROADCAST_ADDRESS),
        )
        .with_context(|| {
            format!(
            ...
            let address = if command == &CliCommands::QueryAddress {
                if *address != r4dcb08_lib::READ_ADDRESS_BROADCAST_ADDRESS {
                    info!(
                        ...
                        address,
                        r4dcb08_lib::READ_ADDRESS_BROADCAST_ADDRESS
                    );
                }
                r4dcb08_lib::READ_ADDRESS_BROADCAST_ADDRESS
            } else {
    ...
    match command {
        ...
        CliCommands::FactoryReset => {
            println!("\
                ...
                You must power off and on again to complete the reset.", r4dcb08_lib::FACTORY_DEFAULT_ADDRESS, BaudRate::from(*r4dcb08_lib::FACTORY_DEFAULT_BAUD_RATE).as_u32()
            );
            let confirmation = Confirm::new()
                .with_prompt("Do you want to continue?")
                .default(false)
